...
class OrganiBajoZero extends Component
{
    ...
    public function render()
    {
        ...
        $users = DB::connection('wordpress')
            ...
            ->whereRaw("(SELECT value FROM dxv_bp_xprofile_data WHERE user_id = dxv_users.ID AND field_id = 50 LIMIT 1) IN ('Bajo Zero', 'Gerencia Administrativa', 'Publicidad')")
            ->where(function ($query) {
                $query->where('t.name', '!=', 'gerente') // Excluir a todos los gerentes
                      ->orWhereRaw("(SELECT value FROM dxv_bp_xprofile_data WHERE user_id = dxv_users.ID AND field_id = 50 LIMIT 1) = 'Bajo Zero'"); // Pero permitir si es gerente de Publicidad
            })
            ->get()
        ...
        return view('livewire.admin.organigramas.organi-bajo-zero', compact('users', 'profileTypes'));
    }    
}
