...
class OrganiPal extends Component
{
    ...
    public function render()
    {
        ...
        $users = DB::connection('wordpress')
            ...
            ->where(function ($query) {
                $query->where('t.name', '!=', 'gerente') // Excluir a todos los gerentes
                      ->orWhereRaw("(SELECT value FROM dxv_bp_xprofile_data WHERE user_id = dxv_users.ID AND field_id = 50 LIMIT 1) = 'PAL'"); // Pero permitir si es gerente de Publicidad
            })
            ->get()
        ...
        return view('livewire.admin.organigramas.organi-pal', compact('users', 'profileTypes'));
    }
}
