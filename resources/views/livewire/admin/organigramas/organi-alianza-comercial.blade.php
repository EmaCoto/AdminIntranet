<x-content-organi>
    <div class="relative">
        <div class="absolute -top-10 z-50 flex justify-center w-full">
            <a href="{{ route('AlianzaComercialLegal') }}" class="p-2 bg-[#B23B3B] text-white rounded-md shadow-md w-fit uppercase font-bold hover:bg-slate-50 hover:text-[#B23B3B] hover:border-[#B23B3B] border-2 border-[#B23B3B]"><i class="fa-solid fa-table pr-2"></i>ver tabla</a>
        </div>
    </div>
    <div class="flex justify-center items-start my-4 w-full relative">
        <div class="tree-container flex flex-col items-center w-full gap-8">
            ...
            <div class="flex justify-center items-center gap-12">
                @foreach (['subgerente-alianza-comercial-y-legal'] as $role)
                    @if (isset($users[$role]))
            ...
            <!-- director -->
            <div class="flex justify-center items-center gap-12">
                @foreach (['director-alianza-comercial-y-legal'] as $role)
                    @if (isset($users[$role]))
                        @foreach ($users[$role] as $user)
                            <div>
                                <x-profile-type  :user="$user"/>
            ...
            <div class="flex justify-center gap-8">
                {{-- subdirector-alianza-comercial-y-legal --}}
                <div class="p-3 bg-slate-200 h-fit rounded-md">
                    <div class="flex justify-center gap-12">
                        @foreach (['subdirector-alianza-comercial-y-legal'] as $role)
                            @if (isset($users[$role]))
                    ...
                    @foreach (['paralegal-quality-ventas-alianza-comercial-y-legal'] as $role)
                        @if (isset($users[$role]))
