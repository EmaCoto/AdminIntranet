<x-content-organi>
    <div class="relative">
        <div class="absolute -top-10 z-50 flex justify-center w-full">
            <a href="{{ route('Pal') }}" class="p-2 bg-[#B23B3B] text-white rounded-md shadow-md w-fit uppercase font-bold hover:bg-slate-50 hover:text-[#B23B3B] hover:border-[#B23B3B] border-2 border-[#B23B3B]"><i class="fa-solid fa-table pr-2"></i>ver tabla</a>
        </div>
    </div>
    <div class="flex justify-center items-start my-4 w-full relative">
        ...
        <div class="flex gap-4">
            ...
            <div class="p-3 bg-slate-200 h-fit rounded-md">
                ...
                <h1 class="text-lg mb-4 py-1 text-slate-800 text-center justify-center font-bold uppercase">Agentes Comerciales PAL</h1>
                    @if (isset($users[$role]))
                        <div class="grid grid-cols-2 items-center gap-2">
                            @foreach ($users[$role] as $user)
                                <x-user-card :user="$user"/>
            ...
            <div class="p-3 bg-slate-200 h-fit rounded-md">
                ...
                <h1 class="text-lg mb-4 py-1 text-slate-800 text-center justify-center font-bold uppercase">Ventas PAL</h1>
                    @if (isset($users[$role]))
                        <div class="grid grid-cols-2 items-center gap-2">
                            @foreach ($users[$role] as $user)
                                <x-user-card :user="$user"/>
            ...
            <div class="p-3 bg-slate-200 h-fit rounded-md">
                ...
                @foreach (['area-legal-pal'] as $role)
                <h1 class="text-lg mb-4 py-1 text-slate-800 text-center justify-center font-bold uppercase">área legal pal</h1>
                    @if (isset($users[$role]))
                        <div class="grid grid-cols-2 items-center gap-2">
                            @foreach ($users[$role] as $user)
                                <x-user-card :user="$user"/>
