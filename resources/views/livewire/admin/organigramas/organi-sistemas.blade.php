    ...
    <div class="flex justify-center items-start my-4 w-full relative">
        <div class="tree-container flex flex-col items-center w-full gap-8">
            ...
            <div class="flex justify-center items-center gap-12">
                @foreach (['subgerente-de-sistemas'] as $role)
                    @if (isset($users[$role]))
            ...
            <!-- Director -->
            <div class="flex justify-center items-center gap-12">
                @foreach (['director-de-sistemas'] as $role)
                    @if (isset($users[$role]))
                        @foreach ($users[$role] as $user)
                            <div>
                                <x-profile-type  :user="$user"/>
            ...
            <div class="flex justify-center gap-8">
                ...
                <div class="p-3 bg-slate-200 h-fit rounded-md">
                    <h1 class="text-lg mb-4 py-1 text-slate-800 text-center justify-center font-bold">Técinos en sistemas</h1>
                    @foreach (['tecnico-en-sistemas'] as $role)
                        @if (isset($users[$role]))
