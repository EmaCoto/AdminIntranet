<x-content-organi>
    <div class="relative">
        <div class="absolute -top-10 z-50 flex justify-center w-full">
            <a href="{{ route('PermisosTrabajo') }}" class="p-2 bg-[#B23B3B] text-white rounded-md shadow-md w-fit uppercase font-bold hover:bg-slate-50 hover:text-[#B23B3B] hover:border-[#B23B3B] border-2 border-[#B23B3B]"><i class="fa-solid fa-table pr-2"></i>ver tabla</a>
        </div>
    </div>
    <div class="flex justify-center items-start my-4 w-full relative">
        <div class="tree-container flex flex-col items-center w-full gap-8">
            ...
            <div class="flex justify-center items-center gap-12">
                @foreach (['subdirector-de-permisos-de-trabajo'] as $role)
                    @if (isset($users[$role]))
            ...
            <div class="flex">
                <div class="flex justify-center gap-8">
                    ...
                    <div class="p-3 bg-slate-200 h-fit rounded-md">
                        <h1 class="text-lg mb-4 py-1 text-slate-800 text-center justify-center font-bold">paralegales</h1>
                        @foreach (['paralegal-permisos-de-trabajo'] as $role)
                            @if (isset($users[$role]))
                    ...
                    {{-- quality  --}}
                    <div class="p-3 bg-slate-200 h-fit rounded-md">
                        <h1 class="text-lg mb-4 py-1 text-slate-800 text-center justify-center font-bold">quality</h1>
                        @foreach (['quality permisos de trabajo'] as $role)
                            @if (isset($users[$role]))
